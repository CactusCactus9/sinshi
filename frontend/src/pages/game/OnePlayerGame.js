...
import "./PongGame.css";
// import profile from "../../images/img.j";
...
const OnePlayerGame = () => {
    ...
    const updateGame = (ctx) => {
        ...
        const newBotY = players.current[1].y += (ball.y - (players.current[1].y + PADDLE_HEIGHT/2)) * 0.066;//align the paddle's center with the ball //* 0.1 to be able to bet the computer/adversary so it won't be = to the ball's y 
        ...
        if (checkCollision(paddle, ball)){
            let angleRad = (ball.y === (paddle.y + PADDLE_HEIGHT/2)) ? 0 : ( ball.velocityY > 0) ? Math.PI/4 : -Math.PI/4;//move the ball to the opposite direction from which it come
